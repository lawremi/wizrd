...
wizrd:::start_vignette_with_api_key("wizrd-cache", "OPENAI_API_KEY")
options(width = 78)
```
...
object. We can use the `output_as()` function to structure the outputs:
...
filtered_agent <- agent |>
    output_as(mtcars)
filtered_agent |>
    predict("First five cars")

# Return a data.frame with specific, renamed columns
summary_agent <- agent |>
...
agent |>
    ...
    predict("First five cars cars including mpg, hp, and wt.")
```
...
straightforward to convert it into an actual R function using `as.function`:
```{r function}
# Convert the filtered agent to a function
filter_cars <- as.function(filtered_agent)
