...
`ollama_model()` to pull (if necessary) and run models with
...
```r
model |> chat("Who created R?") |> predict("When was it created?")
```
...
```r
parameterized_model <- model |> prompt_as("Who created {language}?")
parameterized_model |> predict(list(language = "R"))
```
...
function:
```r
instructed_model <- parameterized_model |>
    instruct("Include all major contributors. Return only names, no other text.")
instructed_model |> predict(list(language = "R"))
```
The model obeys the instructions to include more contributors and to
only return their names.

We can also provide a tool that queries for the authors of the R
language:
```r
get_contributors <- function() {
    R.home("doc") |> file.path("AUTHORS") |> readLines() |> paste(collapse="\n")
}
equipped_model <- instructed_model |> equip(get_contributors)
equipped_model |> predict(list(language = "R"))
```
...
s7_model <- instructed_model |> output_as(Person)
s7_model |> predict(list(language = "R"))
```
...
```r
model_fun <- convert(df_model, class_function)
model_fun("R")
```
...
In support of this, the package implements experimental functionality
...
The above code uses the `chunk` generic to generate text chunks from
...
As a final example, we can query this very vignette:
```r
chunks <- chunk(vignette(package = "wizrd"))
store <- text_store(nomic(), chunks)
model <- llama() |> prompt_as(rag_from(store, k = 1L))
predict(model, "RAG example")
```
And now the vignette has come full circle, querying itself.
